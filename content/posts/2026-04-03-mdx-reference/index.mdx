...
Imagens podem ser inseridas diretamente no Markdown usando o sintaxe abaixo. O caminho é relativo ao arquivo MDX, então `./nome-da-imagem.jpg` se refere a um arquivo na mesma pasta do `index.mdx`.
...
Para imagens com legenda ou controle fino de propriedades, use o componente `Image` do `next/image`.
...
<RatioImage
  ...
  ratio={16 / 9}
  title='Título da imagem. Opcional.'
  caption='Figura 2. Imagem com proporção fixa (16:9). Opcional.'
/>
...
```mdx
<ImageGrid columns={3}>
  <GridImage src={Imagem1} alt='...' title='Título da imagem. Opcional.' />
  <GridImage src={Imagem2} alt='...' title='Título da imagem. Opcional.' />
  <GridImage src={Imagem3} alt='...' title='Título da imagem. Opcional.' />
</ImageGrid>
```
...
O `remark-gemoji` converte shortcodes no emoji correspondente. Use para adicionar ícones inline sem precisar importar imagens ou componentes.
...
Controla a visibilidade do post na listagem pública:
...
Quando o post faz parte de uma série, informe o slug da série e a posição do post dentro dela. Ambos os campos são obrigatórios em conjunto — um não funciona sem o outro:
...
Séries são coleções de posts organizados por tema. Elas são definidas no arquivo `content/series/index.json`:
...
Projetos são gerados automaticamente a partir dos repositórios públicos do GitHub via API (`@octokit/rest`). O controle de quais repositórios aparecem e como é feito pelos **topics** de cada repositório, diretamente na interface do GitHub.
...
As tags exibidas no card são os topics do repositório, excluindo os três de controle acima (`portfolio`, `featured`, `wip`).
