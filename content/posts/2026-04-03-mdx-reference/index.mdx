...
Marca linhas como adicionadas ou removidas usando `// [!code ++]` e `// [!code --]`:
```ts
function greet(name: string): string {
  return `Hello, ${name}!`  // [!code --]
  return `Olá, ${name}!`    // [!code ++]
}
```

#### Focus

Destaca linhas específicas e desfoca as demais. Útil para chamar atenção em um trecho sem remover o contexto ao redor. Passe o cursor sobre o bloco para ver todas as linhas novamente:
```ts
function createUser(data: UserInput) {
  validateInput(data)
  const hashed = hashPassword(data.password)  // [!code focus]
  return db.users.create({ ...data, password: hashed })
}
```

Combine com `showLineNumbers` quando o número de linha ajudar a referenciar o código no texto:
```ts showLineNumbers
function createUser(data: UserInput) {
  validateInput(data)
  const hashed = hashPassword(data.password)  // [!code focus]
...
| Valor       | Comportamento                                               |
| ----------- | ----------------------------------------------------------- |
| `draft`     | Nunca foi publicado — não aparece na listagem               |
| `published` | Visível publicamente                                        |
| `archived`  | Foi publicado, mas foi removido da listagem                 |
